...
import static se.playpark.dhs.configuration.Leaderboard.*;
...
public class Board {
    ...
    private final Map<UUID, Integer> hider_kills = new HashMap<>(), seeker_kills = new HashMap<>(),
            hider_deaths = new HashMap<>(), seeker_deaths = new HashMap<>();
        ...
        for (String line : LOBBY_CONTENTS) {
            if (line.equalsIgnoreCase("")) {
                ...
                if (!lobbyCountdownEnabled) {
                    ...
                    board.setLine(String.valueOf(i), line.replace("{COUNTDOWN}",
                            COUNTDOWN_COUNTING.replace("{AMOUNT}", Main.getInstance().getGame().getLobbyTime() + "")));
                }
    ...
    public String getMapName() {
        se.playpark.dhs.configuration.Map map = Main.getInstance().getGame().getCurrentMap();
        if (map == null)
            return "Invalid";
        else
            return map.getName();
            ...
            if (line.equalsIgnoreCase("")) {
                ...
                if (line.contains("{TIME}")) {
                    ...
                    if (status == Status.STARTING) {
                        board.setLine(String.valueOf(i),
                                line.replace("{BORDER}", BORDER_COUNTING.replace("{AMOUNT}", "0")));
                    } else if (!worldBorder.isRunning()) {
                        board.setLine(String.valueOf(i),
                                line.replace("{BORDER}",
                                        BORDER_COUNTING.replaceFirst("\\{AMOUNT}", worldBorder.getDelay() / 60 + "")
                                                .replaceFirst("\\{AMOUNT}", worldBorder.getDelay() % 60 + "")));
                    } else {
                    ...
                    if (taunt == null || status == Status.STARTING) {
                        ...
                        board.setLine(String.valueOf(i),
                                line.replace("{TAUNT}",
                                        TAUNT_COUNTING.replaceFirst("\\{AMOUNT}", taunt.getDelay() / 60 + "")
                                                .replaceFirst("\\{AMOUNT}", taunt.getDelay() % 60 + "")));
                    } else {
    ...
    private String getTeam(Player player) {
        if (isHider(player))
            return message("HIDER_TEAM_NAME").toString();
        else if (isSeeker(player))
            return message("SEEKER_TEAM_NAME").toString();
        else if (isSpectator(player))
            return message("SPECTATOR_TEAM_NAME").toString();
        else
