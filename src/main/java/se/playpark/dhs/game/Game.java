...
import se.playpark.dhs.game.listener.RespawnHandler;
import se.playpark.dhs.game.util.CountdownDisplay;
...
import se.playpark.dhs.game.util.WinType;
import org.bukkit.*;
...
public class Game {
	...
	public void start(List<Player> seekers) {
		if (mapSaveEnabled)
			currentMap.getWorldLoader().rollback();
		board.reload();
	...
	private void stop(WinType type) {
		...
		if (type == WinType.HIDER_WIN) {
			...
			Main.getInstance().getDatabase().getGameData().addWins(board, players, winners, board.getHiderKills(),
					board.getHiderDeaths(), board.getSeekerKills(), board.getSeekerDeaths(), type);
	...
	public void end() {
		...
		if (mapSaveEnabled)
			currentMap.getWorldLoader().unloadMap();
	...
	public void join(Player player) {
		if (status != Status.STARTING && status != Status.PLAYING) {
			...
			if (announceMessagesToNonPlayers)
				...
				broadcastMessage(messagePrefix + message("GAME_JOIN").addPlayer(player));
		} else {
	...
	public void leave(Player player) {
		...
		if (announceMessagesToNonPlayers)
			...
			broadcastMessage(messagePrefix + message("GAME_LEAVE").addPlayer(player));
	...
	public void onTick() {
		if (currentMap == null || currentMap.isNotSetup())
			return;
		if (status == Status.STANDBY)
			whileWaiting();
	...
	private void whilePlaying() {
			...
			if (seekerPing)
				switch (gameTick % 10) {
					case 0:
						...
						if (distance < seekerPingLevel3)
							ringingSound.play(hider, seekerPingVolume, seekerPingPitch);
						break;
					case 3:
						if (distance < seekerPingLevel1)
							heartbeatSound.play(hider, seekerPingVolume, seekerPingPitch);
						if (distance < seekerPingLevel3)
							ringingSound.play(hider, seekerPingVolume, seekerPingPitch);
						break;
					case 6:
		...
		if (gameTick % 20 == 0) {
			...
			if (currentMap.isWorldBorderEnabled())
				currentMap.getWorldBorder().update();
			if (tauntEnabled)
				taunt.update();
			if (glowEnabled || alwaysGlow)
				glow.update();
		}
		...
		if (hiderCount < 1 || (!waitTillNoneLeft && hiderCount < 2)) {
			if (hiderLeft && dontRewardQuit) {
				if (announceMessagesToNonPlayers)
					...
					broadcastMessage(gameOverPrefix + message("GAME_GAMEOVER_HIDERS_QUIT"));
				if (gameOverTitle)
					broadcastTitle(message("GAME_TITLE_NO_WIN").toString(),
							message("GAME_GAMEOVER_HIDERS_QUIT").toString());
				stop(WinType.NONE);
			} else {
				if (hiderCount < 1 || waitTillNoneLeft) {
					if (announceMessagesToNonPlayers)
						...
						broadcastMessage(gameOverPrefix + message("GAME_GAMEOVER_HIDERS_FOUND"));
					if (gameOverTitle)
						broadcastTitle(message("GAME_TITLE_SEEKERS_WIN").toString(),
								message("GAME_GAMEOVER_HIDERS_FOUND").toString());
				} else {
					...
					if (announceMessagesToNonPlayers)
						...
						broadcastMessage(gameOverPrefix + message("GAME_GAMEOVER_LAST_HIDER").addPlayer(hider));
					if (gameOverTitle)
						broadcastTitle(message("GAME_TITLE_SINGLE_HIDER_WIN").addPlayer(hider).toString(),
								message("GAME_SUBTITLE_SINGLE_HIDER_WIN").addPlayer(hider).toString());
				}
			...
			if (announceMessagesToNonPlayers)
				...
				broadcastMessage(abortPrefix + message("GAME_GAMEOVER_SEEKERS_QUIT"));
			if (gameOverTitle)
				broadcastTitle(message("GAME_TITLE_NO_WIN").toString(),
						message("GAME_GAMEOVER_SEEKERS_QUIT").toString());
			stop(dontRewardQuit ? WinType.NONE : WinType.HIDER_WIN);
		} else if (gameTimer < 1) {
			if (announceMessagesToNonPlayers)
				Bukkit.broadcastMessage(gameOverPrefix + message("GAME_GAMEOVER_TIME"));
			else
				broadcastMessage(gameOverPrefix + message("GAME_GAMEOVER_TIME"));
			if (gameOverTitle)
