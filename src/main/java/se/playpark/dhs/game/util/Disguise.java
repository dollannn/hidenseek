...
import se.playpark.dhs.util.packet.BlockChangePacket;
import se.playpark.dhs.util.packet.EntityTeleportPacket;
...
public class Disguise {
    ...
    private void sendBlockUpdate(Location location, Material material) {
        ...
        Bukkit.getOnlinePlayers().forEach(receiver -> {
            if (receiver.getName().equals(hider.getName()))
                return;
    ...
    private void respawnFallingBlock() {
        block = hider.getLocation().getWorld().spawnFallingBlock(hider.getLocation().add(0, 1000, 0), material,
                (byte) 0);
    ...
    private void respawnHitbox() {
        if (Main.getInstance().supports(11)) {
            hitBox = (AbstractHorse) hider.getLocation().getWorld().spawnEntity(hider.getLocation().add(0, 1000, 0),
    ...
    private void solidifyUpdate(Location lastLocation, int time) {
        ...
        if (time == 0) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstance(),
                    () -> solidifyUpdate(lastLocation, time - 1), 20);
        }
    }
