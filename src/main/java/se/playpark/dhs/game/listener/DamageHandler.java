...
import se.playpark.dhs.game.Game;
...
public class DamageHandler implements Listener {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        // Makes sure that if there was an attacking player, that the event is allowed
        // for the game
        if (attacker != null) {
            ...
            return;
            // If there is no attacker, it most of been by natural causes. If pvp is
            // disabled, and config doesn't allow natural causes, cancel event.
        ...
        if (pvpEnabled && player.getHealth() - event.getFinalDamage() >= 0.5)
            return;
        // Handle death event
        event.setCancelled(true);
        // Play death effect
