...
import static se.playpark.dhs.configuration.Config.glowPowerupItem;
...
public class InteractHandler implements Listener {
    ...
    private void onSpectatorInteract(ItemStack temp, PlayerInteractEvent event) {
        ...
        if (temp.isSimilar(teleportItem)) {
            // int amount = Main.getInstance().getBoard().getHiders().size() +
            // Main.getInstance().getBoard().getSeekers().size();
            // Inventory teleportMenu = Main.getInstance().getServer().createInventory(null,
            // 9*(((amount-1)/9)+1),
            // ChatColor.stripColor(teleportItem.getItemMeta().getDisplayName()));
            // List<String> hider_lore = new ArrayList<>();
            ...
            // List<String> seeker_lore = new ArrayList<>();
    ...
    public static void createSpectatorTeleportPage(Player player, int page) {
        ...
        final Inventory teleportMenu = Main.getInstance().getServer().createInventory(null, 9 * rows,
                ChatColor.stripColor(teleportItem.getItemMeta().getDisplayName()));
