package se.playpark.dhs.game.events;
...
import static se.playpark.dhs.configuration.Config.tauntDelay;
...
public class Taunt {
    ...
    public void update() {
        if (delay == 0) {
            if (running)
                launchTaunt();
            else if (tauntLast || Main.getInstance().getBoard().sizeHider() > 1)
                executeTaunt();
        } else {
            delay--;
    ...
    private void executeTaunt() {
        Optional<Player> rand = Main.getInstance().getBoard().getHiders().stream()
                .skip(new Random().nextInt(Main.getInstance().getBoard().size())).findFirst();
        if (!rand.isPresent()) {
