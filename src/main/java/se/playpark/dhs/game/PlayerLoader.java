...
import static se.playpark.dhs.configuration.Items.HIDER_ITEMS;
import static se.playpark.dhs.configuration.Items.SEEKER_ITEMS;
...
public class PlayerLoader {
    ...
    public static void loadHider(Player player, Map map) {
        ...
        Titles.sendTitle(player, 10, 70, 20, ChatColor.WHITE + "" + message("HIDER_TEAM_NAME"),
    ...
    public static void loadSeeker(Player player, Map map) {
        ...
        loadPlayer(player);
        Titles.sendTitle(player, 10, 70, 20, ChatColor.WHITE + "" + message("SEEKER_TEAM_NAME"),
    ...
    public static void loadSpectator(Player player, Map map) {
        ...
        Titles.sendTitle(player, 10, 70, 20, ChatColor.GRAY + "" + ChatColor.BOLD + "SPECTATING",
    ...
    private static void loadPlayer(Player player) {
        ...
        for (PotionEffect effect : player.getActivePotionEffects()) {
            if (effect.getType().getName().equals("INVISIBILITY")
                    && Main.getInstance().getDisguiser().disguised(player))
                continue;
            player.removePotionEffect(effect.getType());
        }
    ...
    public static void openBlockHuntPicker(Player player, Map map) {
        ...
        Inventory inventory = Main.getInstance().getServer().createInventory(null, slots,
                "Select a Block: " + map.getName());
