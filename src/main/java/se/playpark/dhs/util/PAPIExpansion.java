...
import static se.playpark.dhs.configuration.Config.placeholderError;
import static se.playpark.dhs.configuration.Config.placeholderNoData;
...
public class PAPIExpansion extends PlaceholderExpansion {
    ...
    public String onRequest(OfflinePlayer player, @NotNull String params) {
        ...
        if ((args.length == 2 || args.length == 3) && (args[0].equals("stats") || args[0].equals("rank-place"))) {
            Optional<PlayerInfo> info = this
                    .getPlayerInfo(args.length == 2 ? player.getUniqueId() : database.getNameData().getUUID(args[2]));
            if (info.isPresent()) {
                switch (args[0]) {
                    case "stats":
                        return getValue(info.get(), args[1]);
                    case "rank-place":
                        if (getRanking(args[1]) == null)
                            return placeholderError;
                        Integer count = database.getGameData().getRanking(getRanking(args[1]), player.getUniqueId());
                        if (getValue(info.get(), args[1]).equals("0"))
                            return "-";
                        if (count == null)
        ...
        if ((args[0].equals("rank-score") || args[0].equals("rank-name")) && args.length == 3) {
            ...
            if (place < 1 || getRanking(args[1]) == null)
                return placeholderError;
            ...
            return args[0].equals("rank-score") ? getValue(info, args[1])
