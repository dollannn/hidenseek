package se.playpark.dhs.configuration;

import java.util.ArrayList;
import java.util.List;

import se.playpark.dhs.Main;
import se.playpark.dhs.game.events.Border;
import se.playpark.dhs.util.Location;
import se.playpark.dhs.world.WorldLoader;
import org.bukkit.*;
...
public class Map {
    ...
    private se.playpark.dhs.util.Location spawnPosition = se.playpark.dhs.util.Location.getDefault(),
            lobbyPosition = se.playpark.dhs.util.Location.getDefault(),
            seekerLobbyPosition = se.playpark.dhs.util.Location.getDefault();
    ...
    public void setSpawn(se.playpark.dhs.util.Location pos) {
        this.spawnPosition = pos;
    }

    public void setLobby(se.playpark.dhs.util.Location pos) {
        this.lobbyPosition = pos;
    }

    public void setSeekerLobby(se.playpark.dhs.util.Location pos) {
    ...
    @NotNull
    public se.playpark.dhs.util.Location getGameSpawn() {
        if (mapSaveEnabled) {
    ...
    @NotNull
    public se.playpark.dhs.util.Location getSpawn() {
        return spawnPosition;
    }
    ...
    @NotNull
    public se.playpark.dhs.util.Location getLobby() {
    ...
    @NotNull
    public se.playpark.dhs.util.Location getSeekerLobby() {
    ...
    public boolean isNotSetup() {
        if (spawnPosition.getBlockX() == 0 && spawnPosition.getBlockY() == 0 && spawnPosition.getBlockZ() == 0
                || !spawnPosition.exists())
            return true;
        if (lobbyPosition.getBlockX() == 0 && lobbyPosition.getBlockY() == 0 && lobbyPosition.getBlockZ() == 0
                || !lobbyPosition.exists())
            return true;
        if (exitPosition == null
                || exitPosition.getBlockX() == 0 && exitPosition.getBlockY() == 0 && exitPosition.getBlockZ() == 0
                || !exitPosition.exists())
            return true;
        if (seekerLobbyPosition.getBlockX() == 0 && seekerLobbyPosition.getBlockY() == 0
                && seekerLobbyPosition.getBlockZ() == 0 || !seekerLobbyPosition.exists())
            return true;
        if (mapSaveEnabled && !getGameSpawn().exists())
            return true;
        if (blockhunt && blockhuntBlocks.isEmpty())
            return true;
        if (isWorldBorderEnabled() &&
                new Vector(spawnPosition.getX(), 0, spawnPosition.getZ())
                        .distance(new Vector(xWorldBorder, 0, zWorldBorder)) > 100)
            return true;
        return isBoundsNotSetup();
    }

    public boolean isBoundsNotSetup() {
        if (xBoundMin == 0 || zBoundMin == 0 || xBoundMax == 0 || zBoundMax == 0)
            return true;
