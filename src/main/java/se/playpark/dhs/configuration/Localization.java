	...
	private static final Map<String, String[][]> CHANGES = new HashMap<String, String[][]>() {
		{
			put("en-US", new String[][] {
					...
					{ "START", "TAUNTED" },
					{ "GAME_SETUP", "SETUP_GAME", "SETUP_LOBBY", "SETUP_SEEKER_LOBBY", "SETUP_EXIT", "SETUP_SAVEMAP",
							"SETUP_BOUNDS" },
		...
		for (String key : manager.getConfigurationSection("Localization").getKeys(false)) {
			LOCAL.put(
					...
					new LocalizationString(
							ChatColor.translateAlternateColorCodes('&', manager.getString("Localization." + key))));
		}
	...
	public static LocalizationString message(String key) {
		...
		if (message == null) {
			...
			if (defaultMessage == null) {
				return new LocalizationString(ChatColor.RED + "" + ChatColor.ITALIC + key
						+ " is not found in localization.yml. This is a plugin issue, please report it.");
			}
