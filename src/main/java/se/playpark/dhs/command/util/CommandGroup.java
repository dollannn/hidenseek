package se.playpark.dhs.command.util;

import se.playpark.dhs.configuration.Config;
import se.playpark.dhs.configuration.Localization;
import se.playpark.dhs.util.Pair;
import se.playpark.dhs.util.Tuple;
...
public class CommandGroup {
	...
	public void handleCommand(Player player, String[] args) {
		...
		if (data == null) {
			player.sendMessage(
					String.format("%sDollan's Hide and Seek %s(%s1.0.0%s)\n", ChatColor.AQUA, ChatColor.BOLD,
		...
		int parameterCount = (int) Arrays.stream(command.getUsage().split(" "))
				.filter(p -> p.startsWith("<") && !p.startsWith("<*")).count();
	...
	private Tuple<ICommand, String, String[]> getCommand(String[] args, String permission) {
		...
		if (commandRegister.containsKey(invoke)) {
			...
			if (o instanceof CommandGroup) {
				...
				return new Tuple<>(command, permission + "." + command.getLabel(),
						Arrays.copyOfRange(args, 1, args.length));
			}
		}
		return null;
		...
		if (args.length == 1) {
			return new ArrayList<>(commandRegister.keySet())
					...
					.filter(handle -> {
						...
						if (object instanceof ICommand) {
							ICommand command = (ICommand) object;
							return !Config.permissionsRequired
									|| player.hasPermission(permission + "." + command.getLabel());
						} else if (object instanceof CommandGroup) {
							CommandGroup group = (CommandGroup) object;
							return !Config.permissionsRequired
									|| group.hasPermission(player, permission + "." + group.getLabel());
						}
						return false;
					})
			...
			if (commandRegister.containsKey(invoke)) {
				...
				if (object instanceof CommandGroup) {
					CommandGroup group = (CommandGroup) object;
					return group.handleTabComplete(player, permission + "." + group.getLabel(),
							Arrays.copyOfRange(args, 1, args.length));
	...
	private boolean hasPermission(Player player, String permission) {
		for (Object object : commandRegister.values()) {
			if (object instanceof ICommand) {
				ICommand command = (ICommand) object;
				if (player.hasPermission(permission + command.getLabel()))
					return true;
			} else if (object instanceof CommandGroup) {
				CommandGroup group = (CommandGroup) object;
				if (group.hasPermission(player, permission + this.label + "."))
					return true;
			}
		}
		return false;
